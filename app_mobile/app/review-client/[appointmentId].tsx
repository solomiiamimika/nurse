...
export default function ReviewClientScreen() {
  ...
  const handleSubmit = async () => {
    ...
    setSubmitting(true);
    try {
      await api.post('/provider/leave_review', {
        appointment_id: parseInt(appointmentId, 10),
        rating,
        comment: comment.trim(),
      });

      Alert.alert(t('success'), 'Your review has been submitted.', [
        { text: t('ok'), onPress: () => router.back() },
      ]);
    } catch (err: any) {
      const msg = err.response?.data?.msg || 'Failed to submit review';
      if (msg.toLowerCase().includes('already')) {
        Alert.alert(t('error'), 'You have already reviewed this appointment.');
      } else {
  ...
  return (
    <SafeAreaView style={styles.container} edges={['top']}>
      <Stack.Screen options={{ headerShown: true, title: t('clientReview') }} />
      <KeyboardAvoidingView
      ...
      >
        <ScrollView contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
          <View style={styles.content}>
            <Text style={styles.title}>{t('reviewClient')}</Text>
            ...
            {/* Comment */}
            <Text style={styles.inputLabel}>{t('shareExperience')}</Text>
            <TextInput
